...
class OboeChatbot:
    ...
    def _setup_conversational_chain(self):
        ...
        4.  **정보 출처 명시 (선택적)**: 답변의 신뢰도를 위해, 문서의 어떤 내용을 참고했는지 가볍게 언급할 수 있습니다. 
        (예: "앨범 분석 문서에서는 이 부분을 ~라고 표현하고 있어요.")
        5.  **대화 유도**: 답변 마지막에는 항상 "혹시 더 궁금한 점이 있으신가요?", "이 곡의 다른 부분에 대해서도 이야기해볼까요?" 
        와 같이 자연스럽게 다음 질문을 유도해주세요.
        6.  **모를 때는 솔직하게**: 분석 문서에 없는 내용에 대한 질문을 받으면, "제가 가진 분석 자료에는 해당 내용이 없네요. 죄송하지만 다른 질문을 해주시겠어요?" 
        라고 솔직하고 정중하게 답변하세요. 절대 추측해서 말하지 마세요.
